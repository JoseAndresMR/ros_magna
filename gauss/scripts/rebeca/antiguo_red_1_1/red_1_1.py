...
input_len = 2000#datos_entrada.simulation_len
...
tf.app.flags.DEFINE_integer('train_size', 20000, 'set of training data')

tf.app.flags.DEFINE_integer('valid_size', 5000, 'set of validation data')

tf.app.flags.DEFINE_integer('BATCH_SIZE', 100, 'Must divide evenly into the dataset sizes.')

tf.app.flags.DEFINE_integer('n_hidden_1', 4, 'Number of neuron in layer 1')

tf.app.flags.DEFINE_integer('n_hidden_2', 4, 'Number of neuron in layer 2')

tf.app.flags.DEFINE_integer('n_hidden_3', 10, 'Number of neuron in layer 3')
...
with graph.as_default():
    ...
    loss_regu = beta1*(tf.nn.l2_loss( weights['h1']))+ beta2*(tf.nn.l2_loss(weights['h2']))#+tf.nn.l2_loss(weights['h3']))
    """
