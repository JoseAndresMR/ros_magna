...
input_len = datos_entrada.simulation_len
...
tf.app.flags.DEFINE_integer('train_size', 500, 'set of training data')

tf.app.flags.DEFINE_integer('valid_size', 150, 'set of validation data')

tf.app.flags.DEFINE_integer('BATCH_SIZE', 3, 'Must divide evenly into the dataset sizes.')

tf.app.flags.DEFINE_integer('n_hidden_1', 10, 'Nuumber of neuron in layer 1')

tf.app.flags.DEFINE_integer('n_hidden_2', 10, 'Nuumber of neuron in layer 2')

tf.app.flags.DEFINE_integer('n_hidden_3', 10, 'Nuumber of neuron in layer 3')
...
""" 
train_data =  data_input[0:train_size, :]
train_labels = labels_input[0:train_size, :]
    
valid_data = data_input[train_size: valid_size+train_size, : ]
valid_labels = labels_input[train_size: valid_size+train_size, :]
    
test_data= data_input[valid_size+train_size : input_len, :]
test_labels= labels_input[valid_size+train_size : input_len, :]
...
with graph.as_default():
    ...
    weights = {
        'h1': tf.Variable(tf.truncated_normal([num_features, num_label], stddev = dev_1, dtype= np.float32)),
        #'h2': tf.Variable(tf.truncated_normal([n_hidden_1, n_hidden_2], stddev = dev_2, dtype= np.float32)),
        #'h3': tf.Variable(tf.truncated_normal([n_hidden_2, n_hidden_3], stddev = dev_3, dtype= np.float32)),
        #'out': tf.Variable(tf.truncated_normal([n_hidden_3, num_label], stddev = dev_out, dtype= np.float32))
        }
   
    biases = {
        'b1': tf.Variable(tf.zeros([num_label], dtype= np.float32)),
    ...
    loss_regu = beta1*(tf.nn.l2_loss( weights['h1']))#+ tf.nn.l2_loss(weights['h2']) +tf.nn.l2_loss(weights['h3']))
    """
