...
class adsb(object):
    ...
    def nic_v1(msg):
        ...
        if self.nic_s == None:
            raise RuntimeError("NICs still unknown")
    ...
    def nic_v2(msg,self):
        ...
        if typecode(msg) < 5 or typecode(msg) > 22:
            raise RuntimeError("%s: Not a surface position message (5<TC<8, )airborne position message (8<TC<19), airborne position with GNSS height (20<TC<22)" % msg)
        if self.nic_a == None:
            raise RuntimeError("NICa still unknown")
        if (typecode(msg) > 4 or typecode(msg) < 9) and self.nic_c == None:
            raise RuntimeError("NICc still unknown")

        if (typecode(msg) > 8 or typecode(msg) < 19) and self.nic_a == None:
            raise RuntimeError("NICa still unknown")
    ...
    def nic_s(msg):
        ...
        msgbin = util.hex2bin(msg)
        self.nic_s = util.bin2int(msgbin[75])
    ...
    def nic_a_and_c(msg):
        ...
        msgbin = util.hex2bin(msg)
        self.nic_a = util.bin2int(msgbin[75])
        self.nic_c = util.bin2int(msgbin[51])

        return self.nic_a, self.nic_c

    def nic_b(msg):
        """Calculate NIC, navigation integrity category supplement
        ...
        msgbin = util.hex2bin(msg)
        self.nic_b = util.bin2int(msgbin[39])
    ...
    def nacp(msg):
        ...
        tc = typecode(msg)
        if tc == 29:
            nacp = util.bin2int(msgbin[71:75])
        elif tc == 31:
            nacp = util.bin2int(msgbin[76:80])
        else:
            nacp = -1
        return nacp
    ...
    def nacv(msg):
        ...
        tc = typecode(msg)
        if tc == 19:
            nacv = util.bin2int(msgbin[42:45])
        else:
            nacv = -1
        return nacv
    ...
    def sil(msg,version):
        ...
        tc = typecode(msg)
        if tc == 29:
            sil = util.bin2int(msgbin[76:78])
        elif tc == 31:
            sil = util.bin2int(msg[82:84])
        else:
            sil = -1

        if version == 2:
            if typecode(msg) == 29:
                sils = util.bin2int(msgbin[39])
            elif typecode(msg) == 31:
                sils = util.bin2int(msgbin[86])
        else:
            sils = -1
    ...
    def version(msg):
        ...
        if typecode(msg) in [29,31]:
            version = util.bin2int(msgbin[72:75])
        else:
            version = -1
        return version
